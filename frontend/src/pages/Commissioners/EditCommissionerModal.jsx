...
import { updateCommissioner } from '../../services/commissionerService';

const EditCommissionerModal = ({ commissioner, onClose, onSave }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      await updateCommissioner(commissioner._id, formData);
      setError(null);
      onSave(); // Close modal and refresh list
    } catch (err) {
      console.error('Error updating commissioner:', err);
      setError(err.response?.data?.message || 'Failed to update commissioner. Please try again.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-8 max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-800">Edit Commissioner</h2>
          <button
            onClick={onClose}
...
export default EditCommissionerModal; 