...
import { getBrokers, searchBrokers, deleteBroker } from '../../services/brokerService';
...
const BrokerList = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      setLoading(true);
      const data = await searchBrokers(query);
      setBrokers(data);
      setError(null);
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    if (!isAdmin) {
      alert('Only administrators can delete brokers');
      return;
    }
  ...
  const handleEdit = (broker) => {
    if (!isAdmin) {
      alert('Only administrators can edit brokers');
      return;
    }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-6 text-gray-800">Broker Management</h1>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div className="relative w-full md:w-1/3">
          <input
            ...
            placeholder="Search brokers by name or city..."
            value={searchQuery}
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {loading ? (
                ...
                getPaginatedBrokers().map((broker) => (
