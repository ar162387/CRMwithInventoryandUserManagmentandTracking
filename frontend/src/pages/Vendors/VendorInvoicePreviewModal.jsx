...
const VendorInvoicePreviewModal = ({ invoice, onClose, onSave }) => {
  ...
  const generateInvoiceNumber = () => {
    ...
    const newInvoiceNumber = `VIN-${dateStr}-${randomNum}`;
    setInvoiceNumber(newInvoiceNumber);
  };

  // Calculate totals
  const subtotal = invoice.items.reduce((sum, item) => sum + item.totalPrice, 0);
  const total = subtotal + parseFloat(invoice.labourTransportCost || 0);
  const remainingAmount = total - parseFloat(invoice.paidAmount || 0);
  ...
  const handleSaveInvoice = async () => {
    try {
      ...
      if (!invoiceData.vendorId || invoiceData.vendorId === '') {
        ...
        <div className="print:p-4" id="printable-invoice">
          ...
          <div className="mb-6 border-b pb-4">
            <div className="flex justify-between items-start">
              <div>
                <h3 className="text-lg font-semibold text-gray-800">Vendor Invoice</h3>
              ...
              <div className="text-right">
                <p className="text-gray-600">Vendor: <span className="font-semibold">{invoice.vendorName}</span></p>
                {invoice.brokerName && (
          ...
          <div className="mb-6">
            <div className="bg-gray-50 p-4 rounded-lg w-1/2 ml-auto">
              ...
              <div className="flex justify-between items-center py-2 border-b">
                ...
                <span>{parseFloat(invoice.labourTransportCost || 0).toFixed(2)} PKR</span>
              </div>
              ...
              <div className="flex justify-between items-center py-2 font-bold text-red-600">
                ...
                <span>{remainingAmount.toFixed(2)} PKR</span>
              </div>
            </div>
          </div>
...
export default VendorInvoicePreviewModal; 